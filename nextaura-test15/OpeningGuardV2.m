...
- (void)nextAuraHandleSwitcherPan:(UIPanGestureRecognizer *)pan {
    ...
    if (deliberateTranslation || deliberateVelocity) {
...
static void NAInstallPanObservers(id controller) {
    ...
    for (double delay : (double[]){0.08, 0.22, 0.50}) {
...
static inline void NAClearOnlyNextAura3DEffect(UIView *view) {
    ...
    // SafeSuite writes its switcher style to sublayerTransform. Resetting only
    // this property preserves Apple's stock app-to-switcher scale and motion.
...
__attribute__((constructor)) static void NextAuraInstallOpeningGuardV2(void) {
    @autoreleasepool {
