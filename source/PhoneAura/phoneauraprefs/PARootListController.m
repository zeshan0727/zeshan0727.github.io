...
- (void)viewDidLoad {
    ...
    gradient.startPoint = CGPointMake(0, 0.5); gradient.endPoint = CGPointMake(1, 0.5);
    gradient.frame = CGRectMake(18, 14, UIScreen.mainScreen.bounds.size.width - 54, 108);
    gradient.cornerRadius = 24; gradient.shadowColor = UIColor.blackColor.CGColor;
    gradient.shadowOpacity = 0.24; gradient.shadowRadius = 16; gradient.shadowOffset = CGSizeMake(0, 8);
    ...
    title.text = @"PhoneAura"; title.textColor = UIColor.whiteColor;
    title.font = [UIFont systemFontOfSize:31 weight:UIFontWeightBold]; [header addSubview:title];
    UILabel *subtitle = [[UILabel alloc] initWithFrame:CGRectMake(39, 71, UIScreen.mainScreen.bounds.size.width - 120, 25)];
    subtitle.text = @"Studio App · Full Concept D"; subtitle.textColor = [UIColor colorWithWhite:1 alpha:0.85];
    subtitle.font = [UIFont systemFontOfSize:14 weight:UIFontWeightSemibold]; [header addSubview:subtitle];
    UIImageView *icon = [[UIImageView alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width - 102, 42, 46, 46)];
    icon.image = [UIImage systemImageNamed:@"phone.fill"]; icon.tintColor = UIColor.whiteColor;
    icon.contentMode = UIViewContentModeScaleAspectFit; [header addSubview:icon];
    self.table.tableHeaderView = header;
}

- (void)openStudio {
    NSURL *url = [NSURL URLWithString:@"phoneaura://studio"];
    [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
}

- (void)resetPreferences {
    NSArray *keys = @[@"enabled", @"haptics", @"animations", @"forceDark", @"safeMode",
                      @"showSubtitles", @"showShortcuts", @"styleKeypad", @"hideStockKeypad",
                      @"replaceFavorites", @"replaceContacts", @"replaceRecents", @"replaceVoicemail",
    ...
    CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),
                                         (__bridge CFStringRef)PANotification, NULL, NULL, true);
    [self reloadSpecifiers];
}

- (void)restartPhoneApp {
    pid_t pid; const char *args[] = {"killall", "-9", "MobilePhone", NULL};
