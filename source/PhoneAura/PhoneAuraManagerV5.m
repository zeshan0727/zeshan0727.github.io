...
/*
 * iOS 16 commonly presents Contacts as Lists -> Contacts. Both are treated as
 * the root Contacts tab. Any CNContactViewController, or a deeper stack, is a
 * real system detail screen and must never be covered by PhoneAura.
 */
...
- (void)reloadPreferences {
    ...
    self.favoriteIdentifiers = [identifiers isKindOfClass:[NSArray class]]
        ? identifiers
        : @[];
    ...
    if (self.enabled) {
        [self applyToTabController:self.tabController
                          animated:NO
                  forceDataRefresh:YES];
    } else {
...
- (void)controllerDidAppear:(UIViewController *)controller {
    ...
    if (self.enabled) {
        [self applyToTabController:tabController
                          animated:NO
                  forceDataRefresh:NO];
    } else {
...
- (void)tabSelectionChanged:(UITabBarController *)tabController {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (strongSelf.enabled) {
            [strongSelf applyToTabController:strongTab
                                    animated:YES
                            forceDataRefresh:NO];
        } else {
            [strongSelf restoreTabController:strongTab];
        }
    });
}
...
- (void)installStageForTabController:(UITabBarController *)tabController {
    ...
    if (!stage) {
        ...
        objc_setAssociatedObject(tabController.view,
                                 PAStageKey,
                                 stage,
...
- (void)layoutStageForTabController:(UITabBarController *)tabController {
    ...
    if (stageBottom <= 0.0 || stageBottom > height) {
        stageBottom = height - MAX(49.0, CGRectGetHeight(tabController.tabBar.bounds));
    }

    /*
     * The stage starts at y=0, not below the header. This opaque background is
     * the important overlap fix: Apple's navigation title, segmented controls
     * and first rows can no longer remain visible behind PhoneAura.
     */
    self.stage.frame = CGRectMake(0.0, 0.0, width, MAX(0.0, stageBottom));

    CGFloat headerTop = safeTop + 12.0;
    CGFloat headerHeight = 72.0;
    self.header.frame = CGRectMake(18.0,
                                   headerTop,
                                   MAX(0.0, width - 36.0),
                                   headerHeight);

    CGFloat contentTop = headerTop + headerHeight + 8.0;
    self.contentHost.frame = CGRectMake(0.0,
                                        contentTop,
                                        width,
                                        MAX(0.0, CGRectGetHeight(self.stage.bounds) - contentTop));

    for (UIView *surface in self.contentHost.subviews) {
        surface.frame = self.contentHost.bounds;
    }

    self.dock.frame = CGRectInset(tabController.tabBar.bounds, 8.0, 4.0);
    self.dock.autoresizingMask = UIViewAutoresizingFlexibleWidth |
...
- (void)configureChromeForIndex:(NSUInteger)index animated:(BOOL)animated {
    ...
    [self.dock updateSelectedIndex:index
                          animated:(animated && self.animations)];
}
...
- (void)showSurfaceForIndex:(NSUInteger)index refresh:(BOOL)refresh {
    ...
    if (index == 0 && self.fullFavorites) {
        PAFavoritesDashboardView *surface =
            objc_getAssociatedObject(self.contentHost, PAFavoritesKey);

        if (!surface) {
            surface = [[PAFavoritesDashboardView alloc] initWithFrame:self.contentHost.bounds];

            __weak typeof(self) weakSelf = self;
            surface.callHandler = ^(NSString *number) {
                [weakSelf callNumber:number];
            };
            surface.settingsHandler = ^{
                [weakSelf openStudioApp];
            };

            [self.contentHost addSubview:surface];
            objc_setAssociatedObject(self.contentHost,
                                     PAFavoritesKey,
                                     surface,
                                     OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        ...
        NSString *signature =
            [self.favoriteIdentifiers componentsJoinedByString:@"|"] ?: @"";
        NSString *loadedSignature =
            objc_getAssociatedObject(surface, PAFavoritesSignatureKey);

        if (refresh || ![loadedSignature isEqualToString:signature]) {
            objc_setAssociatedObject(surface,
                                     PAFavoritesSignatureKey,
                                     signature,
        ...
        PARecentsDashboardView *surface =
            objc_getAssociatedObject(self.contentHost, PARecentsKey);
        ...
        PAContactsDashboardView *surface =
            objc_getAssociatedObject(self.contentHost, PAContactsKey);

        if (!surface) {
            surface = [[PAContactsDashboardView alloc] initWithFrame:self.contentHost.bounds];

            __weak typeof(self) weakSelf = self;
            surface.contactHandler = ^(CNContact *contact) {
                [weakSelf showContact:contact];
            };
        ...
        PAStudioKeypadView *surface =
            objc_getAssociatedObject(self.contentHost, PAKeypadKey);
...
- (void)showContact:(CNContact *)contact {
    ...
    CNContactViewController *controller =
        [CNContactViewController viewControllerForContact:contact];
...
- (void)callNumber:(NSString *)number {
    ...
    NSURL *url =
        [NSURL URLWithString:[NSString stringWithFormat:@"tel://%@", escaped ?: clean]];
