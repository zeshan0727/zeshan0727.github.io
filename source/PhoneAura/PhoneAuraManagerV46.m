#import "PhoneAuraManager.h"
#import "PAConceptDUI.h"
#import "PAConceptDSurfaces.h"
#import "PAContactFlowV46.h"
#import <Contacts/Contacts.h>
#import <objc/runtime.h>

static CFStringRef const PADomain = CFSTR("com.zeshan.phoneaura");
static CFStringRef const PAChanged = CFSTR("com.zeshan.phoneaura/preferences.changed");

static const void *PAStageKey = &PAStageKey;
static const void *PAContentHostKey = &PAContentHostKey;
static const void *PAHeaderKey = &PAHeaderKey;
static const void *PADockKey = &PADockKey;
static const void *PAFavoritesKey = &PAFavoritesKey;
static const void *PARecentsKey = &PARecentsKey;
static const void *PAContactsKey = &PAContactsKey;
static const void *PAKeypadKey = &PAKeypadKey;
static const void *PAFavoritePickerKey = &PAFavoritePickerKey;
static const void *PAContactDetailKey = &PAContactDetailKey;
static const void *PAFavoritesSignatureKey = &PAFavoritesSignatureKey;

typedef NS_ENUM(NSUInteger, PAAuxiliaryMode) {
    PAAuxiliaryModeNone = 0,
    PAAuxiliaryModeFavoritePicker,
    PAAuxiliaryModeContactDetail
};

static id PARead(NSString *key) {
    CFPreferencesAppSynchronize(PADomain);
    return CFBridgingRelease(CFPreferencesCopyAppValue((__bridge CFStringRef)key, PADomain));
}

static BOOL PABool(NSString *key, BOOL fallback) {
    id value = PARead(key);
    return value ? [value boolValue] : fallback;
}

static CGFloat PAFloat(NSString *key, CGFloat fallback) {
    id value = PARead(key);
    return value ? [value doubleValue] : fallback;
}

static UINavigationController *PANavigation(UITabBarController *tabController) {
    UIViewController *selected = tabController.selectedViewController;
    return [selected isKindOfClass:[UINavigationController class]] ? (UINavigationController *)selected : nil;
}

static UIViewController *PATop(UITabBarController *tabController) {
    UINavigationController *navigationController = PANavigation(tabController);
    return navigationController ? navigationController.topViewController : tabController.selectedViewController;
}

static UITabBarController *PAFindTab(UIViewController *controller) {
    if (!controller) return nil;
    if ([controller isKindOfClass:[UITabBarController class]]) return (UITabBarController *)controller;
    if (controller.tabBarController) return controller.tabBarController;
    UIViewController *cursor = controller.parentViewController;
    while (cursor) {
        if ([cursor isKindOfClass:[UITabBarController class]]) return (UITabBarController *)cursor;
        if (cursor.tabBarController) return cursor.tabBarController;
        cursor = cursor.parentViewController;
    }
    return nil;
}

static BOOL PAIsSystemDetail(UITabBarController *tabController) {
    UINavigationController *navigationController = PANavigation(tabController);
    if (!navigationController) return NO;
    NSUInteger index = MIN(tabController.selectedIndex, (NSUInteger)4);
    NSUInteger count = navigationController.viewControllers.count;
    if (index == 2) return count > 2;
    return count > 1;
}

static NSString *PANameForContact(CNContact *contact) {
    if (!contact) return @"Contact";
    @try {
        NSMutableArray<NSString *> *parts = [NSMutableArray array];
        if (contact.givenName.length) [parts addObject:contact.givenName];
        if (contact.middleName.length) [parts addObject:contact.middleName];
        if (contact.familyName.length) [parts addObject:contact.familyName];
        NSString *name = [parts componentsJoinedByString:@" "];
        if (name.length) return name;
        if (contact.nickname.length) return contact.nickname;
        if (contact.organizationName.length) return contact.organizationName;
    } @catch (__unused NSException *exception) {
    }
    return @"Contact";
}

static void PASettingsChanged(CFNotificationCenterRef center,
                              void *observer,
                              CFStringRef name,
                              const void *object,
                              CFDictionaryRef userInfo) {
    PhoneAuraManager *manager = (__bridge PhoneAuraManager *)observer;
    dispatch_async(dispatch_get_main_queue(), ^{ [manager reloadPreferences]; });
}

@interface PhoneAuraManager ()
@property(nonatomic) BOOL enabled;
@property(nonatomic) BOOL haptics;
@property(nonatomic) BOOL animations;
@property(nonatomic) BOOL showSubtitles;
@property(nonatomic) BOOL fullFavorites;
@property(nonatomic) BOOL fullRecents;
@property(nonatomic) BOOL fullContacts;
@property(nonatomic) BOOL fullKeypad;
@property(nonatomic) CGFloat cardOpacity;
@property(nonatomic) CGFloat cornerRadius;
@property(nonatomic,strong) NSArray<NSString *> *favoriteIdentifiers;
@property(nonatomic,weak) UITabBarController *tabController;
@property(nonatomic,strong) UIView *stage;
@property(nonatomic,strong) UIView *contentHost;
@property(nonatomic,strong) PAStudioHeaderView *header;
@property(nonatomic,strong) PAStudioDock *dock;
@property(nonatomic) BOOL applying;
@property(nonatomic) NSUInteger lastSelectedIndex;
@property(nonatomic) BOOL hasLastSelectedIndex;
@property(nonatomic) PAAuxiliaryMode auxiliaryMode;
@end

@implementation PhoneAuraManager

+ (instancetype)sharedManager {
    static PhoneAuraManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ manager = [[self alloc] init]; });
    return manager;
}

- (instancetype)init {
    if ((self = [super init])) {
        _lastSelectedIndex = NSNotFound;
        _auxiliaryMode = PAAuxiliaryModeNone;
        [self reloadPreferences];
        CFNotificationCenterAddObserver(CFNotificationCenterGetDarwinNotifyCenter(),
                                        (__bridge const void *)self,
                                        PASettingsChanged,
                                        PAChanged,
                                        NULL,
                                        CFNotificationSuspensionBehaviorDeliverImmediately);
    }
    return self;
}

- (void)dealloc {
    CFNotificationCenterRemoveObserver(CFNotificationCenterGetDarwinNotifyCenter(),
                                       (__bridge const void *)self,
                                       PAChanged,
                                       NULL);
}

- (void)reloadPreferences {
    self.enabled = PABool(@"enabled", YES);
    self.haptics = PABool(@"haptics", YES);
    self.animations = PABool(@"animations", YES);
    self.showSubtitles = PABool(@"showSubtitles", YES);
    self.fullFavorites = PABool(@"fullFavorites", YES);
    self.fullRecents = PABool(@"fullRecents", YES);
    self.fullContacts = PABool(@"fullContacts", YES);
    self.fullKeypad = PABool(@"fullKeypad", YES);
    self.cardOpacity = MAX(0.65, MIN(PAFloat(@"cardOpacity", 0.96), 1.0));
    self.cornerRadius = MAX(10.0, MIN(PAFloat(@"cornerRadius", 16.0), 26.0));
    id identifiers = PARead(@"favoriteIdentifiers");
    self.favoriteIdentifiers = [identifiers isKindOfClass:[NSArray class]] ? identifiers : @[];
    self.hasLastSelectedIndex = NO;
    self.auxiliaryMode = PAAuxiliaryModeNone;
    if (!self.tabController) return;
    if (self.enabled) [self applyToTabController:self.tabController animated:NO forceDataRefresh:YES];
    else [self restoreTabController:self.tabController];
}

- (void)controllerDidAppear:(UIViewController *)controller {
    UITabBarController *tabController = PAFindTab(controller);
    if (!tabController) return;
    self.tabController = tabController;
    if (self.enabled) [self applyToTabController:tabController animated:NO forceDataRefresh:NO];
    else [self restoreTabController:tabController];
}

- (void)controllerDidLayout:(UIViewController *)controller {
    UITabBarController *tabController = PAFindTab(controller);
    if (!self.enabled || !tabController) return;
    self.tabController = tabController;
    if (PAIsSystemDetail(tabController)) {
        [self showSystemNavigation:tabController];
        return;
    }
    NSUInteger index = MIN(tabController.selectedIndex, (NSUInteger)4);
    if (![self customSurfaceEnabledForIndex:index]) {
        [self showSystemNavigation:tabController];
        return;
    }
    [self layoutStageForTabController:tabController];
    [tabController.view bringSubviewToFront:self.stage];
    [tabController.view bringSubviewToFront:tabController.tabBar];
    [tabController.tabBar bringSubviewToFront:self.dock];
}

- (void)tabSelectionChanged:(UITabBarController *)tabController {
    self.tabController = tabController;
    self.auxiliaryMode = PAAuxiliaryModeNone;
    __weak typeof(self) weakSelf = self;
    __weak UITabBarController *weakTab = tabController;
    dispatch_async(dispatch_get_main_queue(), ^{
        PhoneAuraManager *strongSelf = weakSelf;
        UITabBarController *strongTab = weakTab;
        if (!strongSelf || !strongTab) return;
        if (strongSelf.enabled) [strongSelf applyToTabController:strongTab animated:YES forceDataRefresh:NO];
        else [strongSelf restoreTabController:strongTab];
    });
}

- (BOOL)customSurfaceEnabledForIndex:(NSUInteger)index {
    switch (index) {
        case 0: return self.fullFavorites;
        case 1: return self.fullRecents;
        case 2: return self.fullContacts;
        case 3: return self.fullKeypad;
        default: return NO;
    }
}

- (void)applyToTabController:(UITabBarController *)tabController animated:(BOOL)animated forceDataRefresh:(BOOL)forceRefresh {
    if (self.applying || !tabController.view.window) return;
    self.applying = YES;
    if (PAIsSystemDetail(tabController)) {
        [self showSystemNavigation:tabController];
        self.applying = NO;
        return;
    }
    NSUInteger index = MIN(tabController.selectedIndex, (NSUInteger)4);
    if (![self customSurfaceEnabledForIndex:index]) {
        [self showSystemNavigation:tabController];
        self.lastSelectedIndex = index;
        self.hasLastSelectedIndex = YES;
        self.applying = NO;
        return;
    }
    tabController.overrideUserInterfaceStyle = UIUserInterfaceStyleDark;
    [self installStageForTabController:tabController];
    [self installDockForTabController:tabController];
    [self hideStockRootChrome:tabController];
    [self layoutStageForTabController:tabController];
    BOOL changed = !self.hasLastSelectedIndex || self.lastSelectedIndex != index;
    if (changed) self.auxiliaryMode = PAAuxiliaryModeNone;
    if (self.auxiliaryMode == PAAuxiliaryModeNone) {
        [self configureChromeForIndex:index animated:(animated && changed)];
        [self showSurfaceForIndex:index refresh:(forceRefresh || changed)];
    }
    self.lastSelectedIndex = index;
    self.hasLastSelectedIndex = YES;
    self.stage.hidden = NO;
    self.stage.userInteractionEnabled = YES;
    self.dock.hidden = NO;
    [tabController.view bringSubviewToFront:self.stage];
    [tabController.view bringSubviewToFront:tabController.tabBar];
    [tabController.tabBar bringSubviewToFront:self.dock];
    self.applying = NO;
}

- (void)installStageForTabController:(UITabBarController *)tabController {
    UIView *stage = objc_getAssociatedObject(tabController.view, PAStageKey);
    UIView *contentHost = objc_getAssociatedObject(tabController.view, PAContentHostKey);
    PAStudioHeaderView *header = objc_getAssociatedObject(tabController.view, PAHeaderKey);
    if (!stage) {
        stage = [[UIView alloc] init];
        stage.backgroundColor = PAColorHex(0x040817, 1.0);
        stage.opaque = YES;
        stage.clipsToBounds = YES;
        [tabController.view addSubview:stage];
        objc_setAssociatedObject(tabController.view, PAStageKey, stage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    if (!contentHost) {
        contentHost = [[UIView alloc] init];
        contentHost.backgroundColor = PAColorHex(0x040817, 1.0);
        contentHost.opaque = YES;
        contentHost.clipsToBounds = YES;
        [stage addSubview:contentHost];
        objc_setAssociatedObject(tabController.view, PAContentHostKey, contentHost, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else if (contentHost.superview != stage) {
        [stage addSubview:contentHost];
    }
    if (!header) {
        header = [[PAStudioHeaderView alloc] init];
        __weak typeof(self) weakSelf = self;
        header.actionHandler = ^{ [weakSelf headerAction]; };
        [stage addSubview:header];
        objc_setAssociatedObject(tabController.view, PAHeaderKey, header, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else if (header.superview != stage) {
        [stage addSubview:header];
    }
    self.stage = stage;
    self.contentHost = contentHost;
    self.header = header;
    stage.hidden = NO;
    stage.userInteractionEnabled = YES;
    header.hidden = NO;
    [stage bringSubviewToFront:header];
}

- (void)installDockForTabController:(UITabBarController *)tabController {
    UITabBar *tabBar = tabController.tabBar;
    PAStudioDock *dock = objc_getAssociatedObject(tabBar, PADockKey);
    if (!dock) {
        dock = [[PAStudioDock alloc] init];
        __weak typeof(self) weakSelf = self;
        __weak UITabBarController *weakTab = tabController;
        dock.selectionHandler = ^(NSUInteger index) {
            PhoneAuraManager *strongSelf = weakSelf;
            UITabBarController *strongTab = weakTab;
            if (!strongSelf || !strongTab || index >= strongTab.viewControllers.count) return;
            strongSelf.auxiliaryMode = PAAuxiliaryModeNone;
            if (strongSelf.haptics) {
                UISelectionFeedbackGenerator *generator = [[UISelectionFeedbackGenerator alloc] init];
                [generator selectionChanged];
            }
            if (strongTab.selectedIndex == index) [strongSelf applyToTabController:strongTab animated:NO forceDataRefresh:NO];
            else strongTab.selectedIndex = index;
        };
        [tabBar addSubview:dock];
        objc_setAssociatedObject(tabBar, PADockKey, dock, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    self.dock = dock;
    dock.hidden = NO;
}

- (void)hideStockRootChrome:(UITabBarController *)tabController {
    UINavigationController *navigationController = PANavigation(tabController);
    if (navigationController) navigationController.navigationBarHidden = YES;
    UITabBar *tabBar = tabController.tabBar;
    tabBar.backgroundImage = [UIImage new];
    tabBar.shadowImage = [UIImage new];
    tabBar.backgroundColor = UIColor.clearColor;
    tabBar.translucent = YES;
    for (UIView *subview in tabBar.subviews) {
        if (subview == self.dock) continue;
        NSString *className = NSStringFromClass(subview.class);
        if ([subview isKindOfClass:[UIControl class]] || [className containsString:@"Button"]) {
            subview.alpha = 0.0;
            subview.userInteractionEnabled = NO;
        }
    }
    UIViewController *top = PATop(tabController);
    if (top) top.additionalSafeAreaInsets = UIEdgeInsetsZero;
}

- (void)layoutStageForTabController:(UITabBarController *)tabController {
    if (!tabController.view.window || !self.stage || !self.contentHost || !self.header || !self.dock) return;
    CGFloat width = CGRectGetWidth(tabController.view.bounds);
    CGFloat height = CGRectGetHeight(tabController.view.bounds);
    CGFloat safeTop = tabController.view.safeAreaInsets.top;
    CGRect tabFrame = tabController.tabBar.frame;
    CGFloat stageBottom = CGRectGetMinY(tabFrame);
    if (stageBottom <= 0.0 || stageBottom > height) stageBottom = height - MAX(49.0, CGRectGetHeight(tabController.tabBar.bounds));
    self.stage.frame = CGRectMake(0.0, 0.0, width, MAX(0.0, stageBottom));
    CGFloat headerTop = safeTop + 12.0;
    CGFloat headerHeight = 72.0;
    self.header.frame = CGRectMake(18.0, headerTop, MAX(0.0, width - 36.0), headerHeight);
    CGFloat contentTop = headerTop + headerHeight + 8.0;
    self.contentHost.frame = CGRectMake(0.0, contentTop, width, MAX(0.0, CGRectGetHeight(self.stage.bounds)-contentTop));
    for (UIView *surface in self.contentHost.subviews) surface.frame = self.contentHost.bounds;
    self.dock.frame = CGRectInset(tabController.tabBar.bounds, 8.0, 4.0);
    self.dock.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    [self.stage bringSubviewToFront:self.header];
}

- (void)configureChromeForIndex:(NSUInteger)index animated:(BOOL)animated {
    NSArray *titles = @[@"Favorites", @"Recents", @"Contacts", @"Keypad", @"Voicemail"];
    NSArray *subtitles = @[@"Your people, one tap away.", @"Your call history at a glance.", @"All your connections.", @"Dial with confidence.", @"Messages that matter."];
    NSArray *icons = @[@"plus", @"arrow.clockwise", @"arrow.clockwise", @"ellipsis", @"pencil"];
    [self.header configureTitle:titles[index] subtitle:subtitles[index] icon:icons[index] accent:PAAccentForIndex(index) showSubtitle:self.showSubtitles];
    [self.dock updateSelectedIndex:index animated:(animated && self.animations)];
}

- (void)hideAllSurfacesExcept:(UIView *)visible {
    for (UIView *surface in self.contentHost.subviews) {
        surface.hidden = surface != visible;
        surface.userInteractionEnabled = surface == visible;
    }
}

- (void)showSurfaceForIndex:(NSUInteger)index refresh:(BOOL)refresh {
    UIView *visible = nil;
    if (index == 0 && self.fullFavorites) {
        PAFavoritesDashboardV46 *surface = objc_getAssociatedObject(self.contentHost, PAFavoritesKey);
        if (!surface) {
            surface = [[PAFavoritesDashboardV46 alloc] initWithFrame:self.contentHost.bounds];
            __weak typeof(self) weakSelf = self;
            surface.callHandler = ^(NSString *number) { [weakSelf callNumber:number]; };
            surface.chooseHandler = ^{ [weakSelf showFavoritePicker]; };
            [self.contentHost addSubview:surface];
            objc_setAssociatedObject(self.contentHost, PAFavoritesKey, surface, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        surface.hapticsEnabled = self.haptics;
        NSString *signature = [self.favoriteIdentifiers componentsJoinedByString:@"|"] ?: @"";
        NSString *loaded = objc_getAssociatedObject(surface, PAFavoritesSignatureKey);
        if (refresh || ![loaded isEqualToString:signature]) {
            objc_setAssociatedObject(surface, PAFavoritesSignatureKey, signature, OBJC_ASSOCIATION_COPY_NONATOMIC);
            [surface reloadFavoriteIdentifiers:self.favoriteIdentifiers];
        }
        visible = surface;
    } else if (index == 1 && self.fullRecents) {
        PARecentsDashboardView *surface = objc_getAssociatedObject(self.contentHost, PARecentsKey);
        if (!surface) {
            surface = [[PARecentsDashboardView alloc] initWithFrame:self.contentHost.bounds];
            __weak typeof(self) weakSelf = self;
            surface.callHandler = ^(NSString *number) { [weakSelf callNumber:number]; };
            [self.contentHost addSubview:surface];
            objc_setAssociatedObject(self.contentHost, PARecentsKey, surface, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        } else if (refresh) [surface refresh];
        surface.hapticsEnabled = self.haptics;
        visible = surface;
    } else if (index == 2 && self.fullContacts) {
        PAContactsDashboardV46 *surface = objc_getAssociatedObject(self.contentHost, PAContactsKey);
        if (!surface) {
            surface = [[PAContactsDashboardV46 alloc] initWithFrame:self.contentHost.bounds];
            __weak typeof(self) weakSelf = self;
            surface.contactHandler = ^(CNContact *contact) { [weakSelf showContactDetail:contact]; };
            [self.contentHost addSubview:surface];
            objc_setAssociatedObject(self.contentHost, PAContactsKey, surface, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        } else if (refresh) [surface refresh];
        surface.hapticsEnabled = self.haptics;
        visible = surface;
    } else if (index == 3 && self.fullKeypad) {
        PAStudioKeypadView *surface = objc_getAssociatedObject(self.contentHost, PAKeypadKey);
        if (!surface) {
            surface = [[PAStudioKeypadView alloc] initWithFrame:self.contentHost.bounds];
            [self.contentHost addSubview:surface];
            objc_setAssociatedObject(self.contentHost, PAKeypadKey, surface, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
        surface.backgroundColor = PAColorHex(0x040817,1.0);
        surface.hapticsEnabled = self.haptics;
        surface.animationsEnabled = self.animations;
        surface.studioCornerRadius = self.cornerRadius;
        visible = surface;
    }
    [self hideAllSurfacesExcept:visible];
    if (visible) {
        visible.frame = self.contentHost.bounds;
        visible.hidden = NO;
        visible.userInteractionEnabled = YES;
        [self.contentHost bringSubviewToFront:visible];
    }
}

- (void)showFavoritePicker {
    if (!self.contentHost) return;
    self.auxiliaryMode = PAAuxiliaryModeFavoritePicker;
    PAFavoritePickerViewV46 *picker = objc_getAssociatedObject(self.contentHost, PAFavoritePickerKey);
    if (!picker) {
        picker = [[PAFavoritePickerViewV46 alloc] initWithFrame:self.contentHost.bounds];
        __weak typeof(self) weakSelf = self;
        picker.saveHandler = ^(NSArray<NSString *> *identifiers) { [weakSelf saveFavoriteIdentifiers:identifiers]; };
        [self.contentHost addSubview:picker];
        objc_setAssociatedObject(self.contentHost, PAFavoritePickerKey, picker, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    picker.hapticsEnabled = self.haptics;
    [picker reloadSelectedIdentifiers:self.favoriteIdentifiers];
    [self.header configureTitle:@"Choose Favorites" subtitle:@"Select up to four contacts." icon:@"xmark" accent:PAAccentForIndex(0) showSubtitle:YES];
    [self hideAllSurfacesExcept:picker];
    picker.frame = self.contentHost.bounds;
    picker.hidden = NO;
    picker.userInteractionEnabled = YES;
    [self.contentHost bringSubviewToFront:picker];
}

- (void)saveFavoriteIdentifiers:(NSArray<NSString *> *)identifiers {
    NSArray<NSString *> *snapshot = [identifiers isKindOfClass:[NSArray class]] ? [identifiers copy] : @[];
    if (snapshot.count > 4) snapshot = [snapshot subarrayWithRange:NSMakeRange(0,4)];
    self.favoriteIdentifiers = snapshot;
    CFPreferencesSetAppValue(CFSTR("favoriteIdentifiers"), (__bridge CFArrayRef)snapshot, PADomain);
    CFPreferencesAppSynchronize(PADomain);
    self.auxiliaryMode = PAAuxiliaryModeNone;
    [self configureChromeForIndex:0 animated:YES];
    [self showSurfaceForIndex:0 refresh:YES];
}

- (void)showContactDetail:(CNContact *)contact {
    if (!contact || !self.contentHost) return;
    self.auxiliaryMode = PAAuxiliaryModeContactDetail;
    PAContactDetailViewV46 *detail = objc_getAssociatedObject(self.contentHost, PAContactDetailKey);
    if (!detail) {
        detail = [[PAContactDetailViewV46 alloc] initWithFrame:self.contentHost.bounds];
        __weak typeof(self) weakSelf = self;
        detail.callHandler = ^(NSString *number) { [weakSelf callNumber:number]; };
        [self.contentHost addSubview:detail];
        objc_setAssociatedObject(self.contentHost, PAContactDetailKey, detail, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    detail.hapticsEnabled = self.haptics;
    [detail configureContact:contact];
    [self.header configureTitle:PANameForContact(contact) subtitle:@"Contact details inside PhoneAura." icon:@"xmark" accent:PAAccentForIndex(2) showSubtitle:YES];
    [self hideAllSurfacesExcept:detail];
    detail.frame = self.contentHost.bounds;
    detail.hidden = NO;
    detail.userInteractionEnabled = YES;
    [self.contentHost bringSubviewToFront:detail];
}

- (void)closeAuxiliaryMode {
    self.auxiliaryMode = PAAuxiliaryModeNone;
    NSUInteger index = MIN(self.tabController.selectedIndex, (NSUInteger)4);
    [self configureChromeForIndex:index animated:YES];
    [self showSurfaceForIndex:index refresh:NO];
}

- (void)callNumber:(NSString *)number {
    NSCharacterSet *allowed = [NSCharacterSet characterSetWithCharactersInString:@"+0123456789*#"];
    NSMutableString *clean = [NSMutableString string];
    for (NSUInteger index=0; index<number.length; index++) {
        NSString *character = [number substringWithRange:NSMakeRange(index,1)];
        if ([character rangeOfCharacterFromSet:allowed].location != NSNotFound) [clean appendString:character];
    }
    if (!clean.length) return;
    NSString *escaped = [clean stringByAddingPercentEncodingWithAllowedCharacters:NSCharacterSet.URLQueryAllowedCharacterSet];
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"tel://%@", escaped ?: clean]];
    if (url) [UIApplication.sharedApplication openURL:url options:@{} completionHandler:nil];
}

- (void)headerAction {
    if (self.auxiliaryMode != PAAuxiliaryModeNone) {
        [self closeAuxiliaryMode];
        return;
    }
    NSUInteger index = MIN(self.tabController.selectedIndex, (NSUInteger)4);
    if (index == 0) {
        [self showFavoritePicker];
    } else if (index == 1) {
        PARecentsDashboardView *surface = objc_getAssociatedObject(self.contentHost, PARecentsKey);
        [surface refresh];
    } else if (index == 2) {
        PAContactsDashboardV46 *surface = objc_getAssociatedObject(self.contentHost, PAContactsKey);
        [surface refresh];
    } else if (index == 3) {
        PAStudioKeypadView *surface = objc_getAssociatedObject(self.contentHost, PAKeypadKey);
        [surface clearNumber];
    }
}

- (void)showSystemNavigation:(UITabBarController *)tabController {
    self.stage.hidden = YES;
    self.stage.userInteractionEnabled = NO;
    self.dock.hidden = YES;
    UITabBar *tabBar = tabController.tabBar;
    tabBar.backgroundImage = nil;
    tabBar.shadowImage = nil;
    tabBar.backgroundColor = nil;
    tabBar.translucent = YES;
    for (UIView *subview in tabBar.subviews) {
        if (subview == self.dock) continue;
        NSString *className = NSStringFromClass(subview.class);
        if ([subview isKindOfClass:[UIControl class]] || [className containsString:@"Button"]) {
            subview.alpha = 1.0;
            subview.userInteractionEnabled = YES;
        }
    }
    UINavigationController *navigationController = PANavigation(tabController);
    if (navigationController) navigationController.navigationBarHidden = NO;
    UIViewController *top = PATop(tabController);
    if (top) top.additionalSafeAreaInsets = UIEdgeInsetsZero;
    tabController.overrideUserInterfaceStyle = UIUserInterfaceStyleUnspecified;
}

- (void)restoreTabController:(UITabBarController *)tabController {
    [self showSystemNavigation:tabController];
    self.hasLastSelectedIndex = NO;
    self.auxiliaryMode = PAAuxiliaryModeNone;
}

@end
